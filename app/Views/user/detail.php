...
<div class="container">
    <img src="<?= base_url('assets/images/produk/' . $produk['id'] . '/' . $produk['gambar']); ?>" class="img-top" alt="<?= $produk['nama'] ?>" width="500">
    ...
    <div>
        <div>
            <form id="myForm" class="form-outline" method="post" enctype="multipart/form-data">
                ...
                <input class="form-control mr-sm-2" placeholder="ukuran" aria-label="ukuran" value="1" type="number" id="jumlah" name="jumlah" max="5" min="1" />
                <button class="btn btn-primary" type="submit" onclick="setAction('<?= base_url('/user/pembayaran/' . $produk['slug']) ?>')">Beli</button>
                <button class="btn btn-primary" type="submit" onclick="setAction('<?= base_url('/user/keranjang/' . $produk['id']) ?>')">Keranjang</button>
            </form>
